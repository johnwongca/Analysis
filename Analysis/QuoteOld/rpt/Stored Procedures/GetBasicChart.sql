CREATE procedure rpt.GetBasicChart (@Exchange varchar(10) = 'NASDAQ', @Symbol varchar(32) = 'DLLR')
as
begin
	declare @SQL varchar(max) = 'select e.ExchangeID, e.Exchange, s.SymbolID, s.Symbol, s.SymbolName, q.QuoteID, q.Date, q.High, q.[Open], q.[Close], q.Low, q.Volume
	from q.Quote q
						...
						select @SQL [@SQL] ,
							(
								...
								from (values
									...
									('@Symbol', 'varchar', 32, @Symbol)) a([@Name], [@Type], [@Size], [@Value])
	...
	--select @Parameter
	exec [dbo].[CLRGetBasicChart] @Parameter
end
