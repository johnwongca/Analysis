...
namespace FTPLib
{
    public class FTP
    {
        ...
        internal void SetBinaryMode(bool mode)
        {
            if (mode)
                SendCommand("TYPE I");
            else
        ...
        public ArrayList List(string fileName)
        {
        ...
        public ArrayList ListNames(string fileName)
        {
            return List("NLST", fileName);
        }
        public ArrayList List(string listCommand, string listParameter)
        {
            Byte[] bytes = new Byte[512];
            string file_list = "";
            long bytesgot = 0;
            ...
            if (listParameter == "")
                ...
                SendCommand(listCommand + " " + listParameter);
            ReadResponse();
        ...
        public byte[] DownloadToArray(string remotePath, string remoteFilename)
        {
            //Connect();
            ...
            try
            {
                receivedBytes = data_sock.Receive(bytes, offset, data_sock.Available, SocketFlags.Partial);
    ...
    {
        #region Private Variables
        //public FTPLib.FTP ftp;
        Uri mRequestUri = null;
        private FTPLib.FTP ftp;
        private int mRetry = 10, mRetryInterval = 1000;
        FTPFile mRoot, mCurrent;
        ...
        public bool PassiveMode
        {
            get
            {
                return ftp.PassiveMode;
            }
            set
            {
                ftp.PassiveMode = value;
            }
        }
        public FTPFile Root
        {
        ...
        {
            ChangeCurrent(Regex.Split(path, "/"));
            return;
        }
        public void ChangeCurrent(string[] segments)
        {
            ChangeCurrent(new List<string>(segments));
            return;
        }
        public void ChangeCurrent(List<string> segments)
        {
            mCurrent = Root;
            if (mCurrent == null)
                return;
            foreach (string s in segments)
            {
                if (s.Replace("/", "").Trim() == "")
                    continue;
                if (mCurrent.IsFolder)
                {
                    var p = mCurrent.Folders.Where((x) => { return x.Name == s.Replace("/", ""); }).FirstOrDefault();
                    if(p==null)
                    {
                        p = mCurrent.Files.Where((x) => { return x.Name == s.Replace("/", ""); }).FirstOrDefault();
                    }
                    if (p != null)
                        mCurrent = p;
                }
        ...
        #endregion
        #region Constructors
        private void Initialization(Uri uri)
        {
            if (uri.Scheme != System.Uri.UriSchemeFtp)
                throw new Exception("The address entered is not an FTP address.");
            mRequestUri = uri;
            int port = mRequestUri.Port;
            string serverName = mRequestUri.Host;
            string password = "";
            string userName = mRequestUri.UserInfo;
            if (uri.UserInfo.IndexOf(":") > 0)
            {
                userName = Uri.UnescapeDataString(mRequestUri.UserInfo.Substring(0, mRequestUri.UserInfo.IndexOf(":")));
                password = Uri.UnescapeDataString(mRequestUri.UserInfo.Substring(mRequestUri.UserInfo.IndexOf(":") + 1, mRequestUri.UserInfo.Length - mRequestUri.UserInfo.IndexOf(":") - 1));
            }
            ftp = new FTP(serverName, port, userName, password);
        }
        public FTPSite(Uri uri)
        {
            Initialization(uri);
        }
        public FTPSite(string fullName)
        {
            Initialization(new Uri(fullName));
        }
        #endregion
        ...
        public void Connect()
        {
            ftp.Connect();
            //ftp.timeout = 30000;
            mRoot = NewFTPFile("/", true, null);
            mRoot.mPath = "/";
            mRoot.mIsRoot = true;
            mCurrent = mRoot;
            ftp.ChangeDir("/");
            ChangeCurrent(RequestUri.Segments);
        }
        public void Disconnect()
        {
            try
        ...
        private long GetFileSize(string path, string fname)
        {
            for (int retries = 0; retries < Retry - 1; retries++)
            {
                try
                {
                    return ftp.GetFileSize(fname);
                }
                catch
                {
                    try
        ...
        private DateTime GetFileDate(string path, string fname)
        {
            for (int retries = 0; retries < Retry - 1; retries++)
            {
                try
                {
                    return ftp.GetFileDate(fname);
                }
                catch
                {
                    try
        ...
        private ArrayList ListNames(string path)
        {
            for (int retries = 0; retries < Retry - 1; retries++)
            {
                try
        ...
        private byte[] GetFileContent(string path, string fname)
        {
            for (int retries = 0; retries < Retry - 1; retries++)
            {
                try
                {
                    return ftp.DownloadToArray(path, fname);
                }
                catch
                {
                    try
        ...
        private void ReadFolder(FTPFile folder)
        {
            CheckConnectedStatus();
            ftp.SetBinaryMode(true);
            if (!folder.mIsFolder || folder.mFolderPopulated) return;
            folder.mFiles.Clear();
            folder.mFolders.Clear();
            string path = folder.IsRoot ? folder.Path : folder.Path + folder.Name;
            ftp.ChangeDir(path);
            var files = ListNames(path);
            FTPFile f = null;
            string fname;
            foreach (string fileName in files)
            {
                ...
                if (fname == "." || fname == "..")
                    continue;
                f = NewFTPFile(fname, true, folder);
                if (folder.IsRoot)
                    f.mPath = "/";
                else
                    f.mPath = folder.Path + folder.Name + "/";
                f.mSize = GetFileSize(path, fname);
                if (f.mSize != -1)
                {
                    f.mDate = GetFileDate(path, fname);
                    f.mIsFolder = false;
                }
                if (f.mIsFolder)
                    folder.mFolders.Add(f);
                else
                    folder.mFiles.Add(f);
            }
            folder.mFolderPopulated = true;
        }
        private FTPFile NewFTPFile(string fileName, bool isFolder, FTPFile parent)
        {
            FTPFile ret = new FTPFile(fileName, isFolder, parent);
            ret.PopulateFolderHandler += new FTPFileEvent(OnPopulateFolder);
            ret.DownloadHandler += new FTPFileEvent(OnDownload);
            return ret;
        }
        private void OnPopulateFolder(FTPEventArg args)
        {
            ReadFolder(args.File);
        }
        private void OnDownload(FTPEventArg args)
        {
            if (args.File.mIsFolder || args.File.mDataDownloaded) return;
            try
            {
                mLastError = null;
                args.File.mData = GetFileContent(args.File.Path, args.File.Name);
                System.Threading.Thread.Sleep(50);
            }
            catch (Exception e)
            {
    ...
    public class FTPFile
    {
        ...
        internal long mSize = -1;
        internal DateTime mDate = new DateTime(1, 1, 1);
        internal bool mIsFolder = false, mFolderPopulated = false, mIsRoot = false, mDataDownloaded = false;
        internal List<FTPFile> mFiles = new List<FTPFile>();
        internal List<FTPFile> mFolders = new List<FTPFile>();
        internal FTPFile mParent = null;
        internal byte[] mData = null;
        public string Name
        {
            get
            {
            ...
            set
            {
                if (SetNameHandler != null)
                    SetNameHandler(new FTPEventArg(this, value));
            }
        }
        public long Size
        {
        ...
        public List<FTPFile> Files
        {
            get
            {
                if (PopulateFolderHandler != null)
                    PopulateFolderHandler(new FTPEventArg(this, "", ""));
                return mFiles;
            }
        }
        ...
        public byte[] Data
        {
            get
            {
                if (!(mIsFolder || mDataDownloaded))
                {
                    if (DownloadHandler != null)
                        DownloadHandler(new FTPEventArg(this, "", ""));
        ...
        internal FTPFile(string fileName, bool isFolder, FTPFile parent)
        {
            mName = fileName;
            mIsFolder = isFolder;
            mIsRoot = false;
        ...
        internal event FTPFileEvent SetNameHandler;
        internal event FTPFileEvent DownloadHandler;
        internal event FTPFileEvent PopulateFolderHandler;
    }
    internal class FTPEventArg : EventArgs
    {
        public readonly FTPFile File;
        public readonly string NewName;
        public readonly string UploadDownloadFileName;
        public FTPEventArg(FTPFile f)
            : this(f, "")
        {
        }
        public FTPEventArg(FTPFile f, string newName)
            : this(f, newName, "")
        {
        }
        public FTPEventArg(FTPFile f, string newName, string uploadDownloadFileName)
        {
            File = f;
            NewName = newName;
    ...
    internal delegate void FTPFileEvent(FTPEventArg args);
}