...
namespace Console
{
    public class Price
    {
        ...
        {
            get { return mTypicalClosing; }
            set { mTypicalClosing = value; }
        }
        public double WeightedClosing
        {
            get { return mWeightedClosing; }
            set { mWeightedClosing = value; }
        }
        public double MedianPrice
        {
            get { return mMedianPrice; }
            set { mMedianPrice = value; }
        }
        public void ReCalc()
        {
            mTypicalClosing = (mHigh + mLow + mClosing) / Convert.ToDouble(3);
            mWeightedClosing = (mHigh + mLow + mClosing * 2) / Convert.ToDouble(4);
            mMedianPrice = (mHigh + mLow) / Convert.ToDouble(2);
        }
    ...
    public class Analysis
    {
        ...
        public static double ExponentialAverage(double[] data, int period)
        {
            ...
            for (int i = (data.Length - back -1 >=0 ? data.Length - back -1 : 0);i < data.Length; i++)
            {
                ...
                ret = alpha * data[i] + (1.000000 - alpha) * ret;
            }
            return ret;
        }
