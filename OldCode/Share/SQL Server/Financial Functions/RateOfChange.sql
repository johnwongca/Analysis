use Stock
go
if OBJECT_ID('f.RateOfChange') is not null
	drop function f.RateOfChange;
go
/*ID column of @Data must be sequential number*/
create function f.RateOfChange (@Data f.Array1 readonly, @NShort float, @NLong float)
...
insert into @Data(V1, info) 
	...
	where b.Name = 'DLLR'
	order by Date asc
select * from f.RateOfChange(@Data, 10, 28) a inner join @Data b on a.ID = b.ID
